...
def load_article_chunks_v2_corpus(chunks_v2_cache_path):
    with open(chunks_v2_cache_path, 'r', encoding='utf-8') as f:
        data = json.load(f)
    chunks = data['chunks']
...
def load_cached_v2_embeddings_gpu(bgem3_pkl_path, e5_pkl_path, device='cuda'):
    print(f"Loading cached BGE-M3 v2 chunk embeddings from {bgem3_pkl_path}...", flush=True)
    ...
    print(f"Loading cached E5-Large v2 chunk embeddings from {e5_pkl_path}...", flush=True)
...
def eval_exp011e(bgem3_model, e5_model, train_file, folds_file, chunk_doc_ids, bgem3_gpu, e5_gpu, doc_ids_arr, bm25, top_k=5, candidate_top_k=100, alpha=0.3, beta=0.02):
    ...
    print("\n--- Evaluating EXP-011e (Scope/Chapter Chunks v2 + Tight Consensus Window) on 5-Fold Local CV ---", flush=True)
    ...
    print(f"\n=================================================", flush=True)
    print(f"EXP-011e Local 5-Fold CV Mean Recall@{top_k}    : {mean_recall:.4f}", flush=True)
    print(f"EXP-011e Local 5-Fold CV Mean Precision@{top_k} : {mean_precision:.4f}", flush=True)
    print(f"=================================================\n", flush=True)
...
def generate_public_submission(bgem3_model, e5_model, public_file, chunk_doc_ids, bgem3_gpu, e5_gpu, doc_ids_arr, bm25, output_dir, mean_recall, mean_precision, top_k=5, candidate_top_k=100, alpha=0.3, beta=0.02):
    ...
    metrics_text = (
        f"Experiment: EXP-011e (Scope/Chapter Chunks v2 + Tight Consensus Window)\n"
        f"Category: hierarchical/parent_child_consensus_scope_optimized\n"
        f"Description: Scope & Chapter enriched Chunks v2 evaluated with tight candidate window K=100.\n"
...
if __name__ == "__main__":
    ...
    output_dir = os.path.join(base_dir, "results/hierarchical/parent_child_consensus_scope_optimized")
    ...
    chunks, chunk_ids, chunk_doc_ids, doc_to_chunk_ids = load_article_chunks_v2_corpus(cache_chunks_v2)
    ...
    mean_recall, mean_precision = eval_exp011e(
