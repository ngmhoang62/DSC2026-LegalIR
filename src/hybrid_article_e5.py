...
def load_article_chunks_corpus(cache_chunks_path: str):
    ...
    chunk_texts = [f"passage: {c['chunk_text']}" for c in chunks]
    chunk_doc_ids = [c['doc_id'] for c in chunks]
    
    return chunks, chunk_ids, chunk_texts, chunk_doc_ids, doc_to_chunk_ids

def get_or_build_e5_chunk_embeddings(model, chunk_texts, cache_emb_path: str, batch_size: int = 32):
    if os.path.exists(cache_emb_path):
        print(f"Loading E5-Large chunk embeddings from cache: {cache_emb_path}", flush=True)
        with open(cache_emb_path, 'rb') as f:
            chunk_embeddings = pickle.load(f)
    else:
        print(f"Encoding {len(chunk_texts)} Article Chunks with Multilingual-E5-Large (FP16) on local GPU...", flush=True)
        ...
        print(f"Saved E5-Large chunk embeddings to {cache_emb_path}", flush=True)
...
def eval_hybrid_e5_chunked(model, train_path: str, folds_path: str, chunk_doc_ids: list, chunk_embeddings: np.ndarray, doc_ids: list, bm25: BM25Okapi, top_k: int = 5):
    ...
    print("\n--- Evaluating Hybrid BM25 + Multilingual-E5-Large (Article Chunks FP16) on 5-Fold Local CV ---", flush=True)
    for fold_name, val_qids in folds.items():
        y_true = {qid: train_data[qid]['answer'] for qid in val_qids}
        y_pred = {}

        val_queries = [f"query: {train_data[qid]['question']}" for qid in val_qids]
        print(f"  Encoding {len(val_queries)} validation queries in batch on GPU for {fold_name}...", flush=True)
    ...
    print(f"\n>>> EXP-009 (E5-Large Article Chunks FP16) 5-Fold CV Mean Recall@{top_k}: {mean_recall:.4f} | Mean Precision@{top_k}: {mean_precision:.4f} <<<\n", flush=True)
...
def generate_public_submission(model, public_path: str, chunk_doc_ids: list, chunk_embeddings: np.ndarray, doc_ids: list, bm25: BM25Okapi, res_dir: str, mean_recall: float, mean_precision: float, top_k: int = 5):
    ...
    public_queries = [f"query: {public_data[qid]['question']}" for qid in public_qids]
    ...
    print("Generating Public Test predictions for Hybrid BM25 + Multilingual-E5-Large (Article Chunks FP16)...", flush=True)
    ...
    metrics_content = f"""Experiment: EXP-009 (Hybrid BM25 + Multilingual-E5-Large on Article-Level Chunks FP16)
Category: hybrid/e5_article_chunks
Description: Hybrid BM25 (underthesea) + intfloat/multilingual-e5-large FP16 dense retrieval on 276,230 Article-level chunks.
-------------------------------------------------
...
if __name__ == "__main__":
    ...
    res_dir = os.path.join(base_dir, "results/hybrid/e5_article_chunks")

    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    print(f"Using device: {device}", flush=True)

    print("Loading Multilingual-E5-Large model in FP16 half precision for local GPU acceleration...", flush=True)
    model = SentenceTransformer('intfloat/multilingual-e5-large', device=device).half()
    model.max_seq_length = 512

    chunks, chunk_ids, chunk_texts, chunk_doc_ids, doc_to_chunk_ids = load_article_chunks_corpus(cache_chunks)
    chunk_embeddings = get_or_build_e5_chunk_embeddings(model, chunk_texts, cache_e5_emb, batch_size=32)

    doc_ids, bm25 = get_bm25_index(selected_contexts_dir, cache_bm25)

    mean_recall, mean_precision = eval_hybrid_e5_chunked(model, train_file, folds_file, chunk_doc_ids, chunk_embeddings, doc_ids, bm25, top_k=5)
