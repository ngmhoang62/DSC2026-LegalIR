"""Evidence capsule construction for the short EXP-013b Qwen lists."""
...
from exp013b_core import QWEN_INSTRUCTION, hash_payload, load_document_metadata, write_manifest
...
def _render(tokenizer: Any, query: str, document_label: str, chunks: list[dict[str, Any]], *, max_length: int,
            scope_text: str = "") -> tuple[str, str]:
    query_value = _truncate(tokenizer, query, 128)
    first = chunks[0]
    fields = [f"[Văn bản] {_truncate(tokenizer, document_label, 48)}"]
    if scope_text.strip():
    ...
    wrapper = QWEN_PREFIX + f"<Instruct>: {QWEN_INSTRUCTION}\n<Query>: {query_value}\n<Document>: "
    wrapper_tokens = len(tokenizer(wrapper + QWEN_SUFFIX, add_special_tokens=False)["input_ids"])
    budget = max(1, max_length - wrapper_tokens)
    document = _truncate(tokenizer, document, budget)
    while len(tokenizer(wrapper + document + QWEN_SUFFIX, add_special_tokens=False)["input_ids"]) > max_length:
        budget -= 1
        if budget <= 0: raise ValueError("Qwen prompt metadata exceeds capsule token budget")
        document = _truncate(tokenizer, document, budget)
...
class ScopedEvidenceFallback:
    ...
    def __init__(self, *, wanted_docs: set[str], ranking_dir: Path, bge_dir: Path, bm25_db: Path, logger: Any | None = None):
        if logger is not None: logger.log(f"phase=scoped-fallback-index documents={len(wanted_docs)}")
        ...
        for row in read_jsonl(bge_dir / "leaf_rows.jsonl"):
            ...
            if doc_id in wanted_docs:
                index = int(row["row"]); self.rows[index] = row; self.doc_rows[doc_id].append(index)
        self.bm25 = BM25Searcher(bm25_db, profile="legal_structure")
        if logger is not None: logger.log("phase=scoped-fallback-bm25-ranges")
        self.bm25.load_document_ranges()
        self.bm25_query: dict[str, str] = {}
        if logger is not None: logger.log("phase=scoped-fallback-ready")

    def close(self) -> None:
        self.bm25.close()

    def __enter__(self): return self
    def __exit__(self, *_: object): self.close()
    ...
    def select(self, *, qid: str, query: str, doc_id: str) -> list[tuple[str, str]]:
        ...
        if indices and qid in self.query_row:
            ...
            seen: set[str] = set()
            for local in order:
                row = self.rows[indices[int(local)]]; parent = str(row["parent_node_id"])
                if parent not in seen:
                    dense.append((str(row["chunk_id"]), parent, float(scores[int(local)]))); seen.add(parent)
                if len(dense) == 3: break
        expression = self.bm25_query.get(qid)
        if expression is None:
            expression = self.bm25.prepare_query(query); self.bm25_query[qid] = expression
        lexical = self.bm25.search_document_expression(expression, doc_id, limit=8)
        selected: list[tuple[str, str]] = []
        if dense: selected.append((dense[0][0], dense[0][1]))
        primary_parent = selected[0][1] if selected else None
        bm25 = next((row for row in lexical if str(row["parent_node_id"]) != primary_parent), None)
        if bm25 is not None: selected.append((str(bm25["chunk_id"]), str(bm25["parent_node_id"])))
        elif len(dense) > 1: selected.append((dense[1][0], dense[1][1]))
        return selected[:2]
...
def build_capsules(*, split: str, candidates_path: Path, preranker_path: Path, rankings_path: Path,
                   ...
                   tokenizer_name: str, allow_download: bool, max_length: int = 768, shortlist_k: int = 32,
                   resume: bool = False, ranking_dir: Path | None = None, bge_dir: Path | None = None,
                   bm25_db: Path | None = None) -> dict[str, Any]:
    from transformers import AutoTokenizer
    tokenizer = AutoTokenizer.from_pretrained(tokenizer_name, local_files_only=not allow_download, padding_side="left", trust_remote_code=True)
        ...
        with (fallback_context if fallback_context is not None else nullcontext()) as scoped, PersistentChunkReader(v3_dir, lookup_db) as reader:
            for position, qid in enumerate(sorted(candidates), 1):
                ...
                for rank_row in selected:
                    ...
                    if not ids: raise RuntimeError(f"no capsule evidence qid={qid} doc={doc_id}")
                ...
                existing[qid] = {"schema_version": "legalir.exp013b.capsules.v1", "v3_fingerprint": v3_fingerprint,
                                "build_fingerprint": build_fingerprint, "qid": qid, "candidates": rows}
                if position % 64 == 0 or position == len(candidates): logger.status(stage="build-capsules", state="RUNNING", completed=position, total=len(candidates))
