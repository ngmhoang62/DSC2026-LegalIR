"""Candidate generation and integrity audit for EXP-013b."""
...
from exp012b_core import atomic_json, load_answers, load_queries, read_jsonl, sha256_file, stage_run, write_jsonl
from exp012b_tuning import load_folds
from exp013b_core import SCHEMA, hash_payload, load_document_metadata, oracle, write_manifest
...
def build_candidates(*, split: str, rankings_path: Path, memory_path: Path, v3_dir: Path, output_dir: Path,
    ...
    with stage_run(output_dir, "build-candidates", total=len(source), v3_fingerprint=v3_fingerprint) as logger:
        ...
        for position, qid in enumerate(sorted(source), 1):
            ...
            channels = {
                "bge": _source_value(source_row, "bge_leaf", 120),
                "bm25": _source_value(source_row, "bm25", 50),
                "memory": {str(value["doc_id"]): dict(value) for value in memory[qid].get("rankings", [])[:10]},
            }
            ...
            candidates.sort(key=lambda item: (-item["channel_count"], item["best_source_rank"], item["bge_rank"], item["bm25_rank"], item["memory_rank"], item["doc_id"]))
            for rank, item in enumerate(candidates, 1):
                item["rank"] = rank
            rows.append({"schema_version": SCHEMA, "v3_fingerprint": v3_fingerprint, "qid": qid,
                         "query": str(source_row["query"]), "source_fingerprint": ranking_fingerprint,
        ...
        return write_manifest(output_dir, stage="build-candidates", v3_fingerprint=v3_fingerprint,
            config={"split": split, "sources": {"bge_leaf": 120, "bm25": 50, "query_memory": 10}}, files=[result_path],
...
def audit_candidates(*, candidates_path: Path, memory_path: Path, train_path: Path, folds_path: Path,
    ...
    sizes: list[int] = []
    seen: set[str] = set()
    source_fingerprints: set[str] = set(); memory_fingerprints: set[str] = set()
    for record in records:
        qid = str(record["qid"])
        if qid in seen: errors.append(f"duplicate_qid:{qid}")
        seen.add(qid)
        if record.get("v3_fingerprint") != v3_fingerprint: errors.append(f"stale:{qid}")
        ...
        if len(docs) != len(set(docs)): errors.append(f"duplicate_document:{qid}")
        expected_order = sorted(record["candidates"], key=lambda item: (-int(item["channel_count"]), int(item["best_source_rank"]), int(item["bge_rank"]), int(item["bm25_rank"]), int(item["memory_rank"]), str(item["doc_id"])))
        ...
        if set(docs) - v3_docs: errors.append(f"missing_v3_document:{qid}")
        if qid in fold_for:
    ...
    if set(seen) != set(answers): errors.append("query_set_mismatch")
    if len(source_fingerprints) != 1 or "" in source_fingerprints: errors.append("source_fingerprint_mismatch")
    if len(memory_fingerprints) != 1 or "" in memory_fingerprints: errors.append("memory_fingerprint_mismatch")
    missing_ground_truth = sorted({doc_id for values in answers.values() for doc_id in values} - v3_docs)
    if missing_ground_truth: errors.append("ground_truth_missing_v3:" + ",".join(missing_ground_truth[:5]))
    metrics = oracle(records, answers)
    if metrics["recall@180"] < 0.990: errors.append(f"pool_recall={metrics['recall@180']:.6f}")
    if float(np.mean(sizes)) > 145: errors.append("mean_pool_over_145")
    if max(sizes, default=0) > 180: errors.append("max_pool_over_180")
    output_dir.mkdir(parents=True, exist_ok=True)
    report_path = output_dir / "candidate_audit.json"
    report = {"schema_version": SCHEMA, "v3_fingerprint": v3_fingerprint, "status": "PASS" if not errors else "FAIL",
              "errors": errors, "metrics": metrics, "pool_size": {"mean": float(np.mean(sizes)), "max": max(sizes), "min": min(sizes)},
    ...
    with stage_run(output_dir, "audit-candidates", total=len(records), v3_fingerprint=v3_fingerprint) as logger:
        ...
        if errors: raise RuntimeError("candidate audit failed: " + ", ".join(errors[:5]))
        logger.log(f"Recall@180={metrics['recall@180']:.6f} mean_pool={np.mean(sizes):.2f}")
        return write_manifest(output_dir, stage="audit-candidates", v3_fingerprint=v3_fingerprint, config={"minimum_recall": 0.990}, files=[report_path], counts={"queries": len(records)})
