...
def load_cached_embeddings_gpu(bgem3_cache_path: str, e5_cache_path: str, device: str = 'cuda'):
...
def compute_parent_consensus_scores(top_chunk_indices, sims, chunk_doc_ids_arr, 
    ...
    """
    Computes Parent-Child Consensus Scores for documents based on retrieved chunks.
    Score(D) = S_max + alpha * S_top3_mean + beta * log(1 + N_hits)
    """
    doc_sims_map = {}
...
def eval_parent_child_consensus(bgem3_model, e5_model, train_path: str, folds_path: str, 
    ...
    print("\n--- Evaluating EXP-011b (GPU Accelerated Parent-Child Structural Consensus Retrieval) on 5-Fold Local CV ---", flush=True)
    ...
    print(f"\n>>> EXP-011b (Parent-Child Consensus) 5-Fold CV Mean Recall@{top_k}: {mean_recall:.4f} | Mean Precision@{top_k}: {mean_precision:.4f} <<<\n", flush=True)
...
def generate_public_submission(bgem3_model, e5_model, public_path: str, 
    ...
    print("Generating Public Test predictions for EXP-011b (Parent-Child Consensus)...", flush=True)
    ...
    metrics_content = f"""Experiment: EXP-011b (Parent-Child Structural Consensus Retrieval)
Category: hierarchical/parent_child_consensus
Description: Bottom-Up Parent-Child Consensus Retrieval combining Article Chunks with Parent Document Consensus Scoring (MaxP + Top3-Mean + Log(Hits) Consensus).
...
if __name__ == "__main__":
    ...
    res_dir = os.path.join(base_dir, "results/hierarchical/parent_child_consensus_base")

    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    print(f"Using device for query encoding and matrix multiplication: {device}", flush=True)
    ...
    mean_recall, mean_precision = eval_parent_child_consensus(
