"""Shared deterministic infrastructure for EXP-013b Selective Legal Cascade."""
...
SCHEMA = "legalir.exp013b_cascade.v1"
QWEN_MODEL = "Qwen/Qwen3-Reranker-0.6B"
QWEN_INSTRUCTION = (
    "Given a Vietnamese legal question, determine whether the legal document contains provisions "
    "that directly answer the question. Prioritize exact legal conditions, subjects, exceptions, "
    "penalties, procedures, and scope of application."
)
...
def require_success(path: Path, *, v3_fingerprint: str) -> dict[str, Any]:
    ...
    for name, expected in manifest.get("artifact_sha256", {}).items():
        ...
        if not artifact.exists() or sha256_file(artifact) != expected:
            raise RuntimeError(f"Stale/corrupt EXP-013b artifact: {artifact}")
    return payload
...
def stable_order(rows: Sequence[Mapping[str, Any]], *, score_key: str, id_key: str = "doc_id") -> list[dict[str, Any]]:
    return [dict(row) for row in sorted(rows, key=lambda row: (-float(row[score_key]), str(row[id_key])))]
...
def oracle(records: Iterable[Mapping[str, Any]], answers: Mapping[str, set[str]], *, field: str = "candidates") -> dict[str, float]:
    ...
    return evaluate_rankings(predictions, answers, ks=(5, 10, 20, 24, 30, 32, 50, 120, 180))
