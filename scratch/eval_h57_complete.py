...
def apply_enhanced_dedup_and_specialized_kinship(rankings, qids):
    out = {}
    cnt_dedup = 0
    cnt_qd595 = 0
    cnt_corp = 0
    ...
    for q in qids:
        ...
        # 1. Superseded statute de-duplication
        dropped = False
        ...
        if QD595_ID not in top5 and len(preds) > 5 and preds[5] == QD595_ID:
            ...
            if any(p in qtext for p in QD595_PHRASES):
                ...
                if not (lbl5.startswith('luat') or lbl5.startswith('bo luat') or 'sua doi' in lbl5 or 'bo sung' in lbl5):
                    # Swap Rank 5 and Rank 6
                    cand = preds.pop(5)
                    preds.insert(4, cand)
                    cnt_qd595 += 1
                    
        # 3. Exact corporate entity promotion
        top5 = preds[:5]
        qtext = strip_accents(questions.get(q, ''))
        corp_match = None
        ...
        out[q] = preds
    return out, cnt_dedup, cnt_qd595, cnt_corp

# Run full 5-fold CV
all_preds = {}
fold_recalls = []
tot_d, tot_595, tot_c = 0, 0, 0
...
for f in range(5):
    ...
    f_final, cd, c595, cc = apply_enhanced_dedup_and_specialized_kinship(f9, f_eval_qids)
    tot_d += cd
    tot_595 += c595
    tot_c += cc
...
print("="*60)
print(f"H57 Complete Kinship Pipeline:")
...
print(f"Promotions: Dedup={tot_d}, QD595={tot_595}, Corp={tot_c}")
